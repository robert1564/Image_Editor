...
const applyFilter = () => {
    ...
    previewImg.style.filter = `brightness(${brightness}%) saturate(${saturation}%) invert(${inversion}%)
                               grayscale(${grayscale}%) contrast(${contrast}%) sepia(${sepia}%)
                               opacity(${opacity}%) blur(${blur}px) hue-rotate(${hueRotate}deg)
                               drop-shadow(${dropShadow}px ${dropShadow}px ${dropShadow}px)`;
}
...
filterOptions.forEach(option => {
    option.addEventListener("click", () => {
        ...
        if(option.id === "brightness") {
            ...
            filterSlider.max = "40";
            filterSlider.value = dropShadow;
            filterValue.innerText = `${dropShadow}px`
        }
    });
});
